...
from wwwpy.remote.designer.ui.design_aware import to_locator_event, LocatorEvent
...
def default_add_element(locator: Locator, position: Position, element_def):
    file = modlib._find_module_path(locator.class_module)
    old_source = file.read_text()

    path_index = path_to_index(locator.path)
    add_result = add_element(old_source, locator.class_name, element_def, path_index, position)

    if isinstance(add_result, AddResult):
        logger.debug(f'write_module_file len={len(add_result.source_code)} el_path={locator}')
        injector.get(CanvasSelection).current_selection = dataclasses.replace(locator, path=add_result.node_path)

        async def write_source_file():
            from wwwpy.server.designer import rpc
            write_res = await rpc.write_module_file(locator.class_module, add_result.source_code)
...
class AddElementIntent(Intent):
    ...
    add_element: callable = field(default_factory=lambda: default_add_element)
    ...
    def on_hover(self, locator_event: LocatorEvent):
        ...
        tool.set_reference_geometry2(
            ...
            self._get_position(locator_event)
        )
        tool.show()
    ...
    def on_submit(self, locator_event: LocatorEvent) -> bool:
        ...
        if locator_source is not None:
            logger.debug(ep_log + '\n' + message)
            position = self._get_position(locator_event)
...
def default_add_element(self, target: js.HTMLElement, position: Position):
    ep_live = locator_from(target)
    ep_source = _rebase_element_path_to_origin_source(ep_live)
    ep_log = \
        f'Locator live: {ep_live} position: {position}' + '\n' + \
